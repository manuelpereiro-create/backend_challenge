...
exports.getUserMetrics = async (request, response) => {
    try {
        const userId = request.user.id; // Obtener el ID del usuario desde el token JWT
        const [rows] = await db.query('SELECT login_count, last_login FROM users WHERE id = ?', [userId]);
